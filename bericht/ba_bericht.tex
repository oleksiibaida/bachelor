...
\begin{enumerate}
  ...
  \item \textbf{Automatisierung}: Basierend auf den erfassten Sensordaten können bestimmte Vorgänge automatisch ausgelöst werden.
\end{enumerate}
\par 
\newpage
...
% 
    ...
    async def start_client(cls, topics: list):
      while True:
        try:
          ...
          logger.error(e)
  \end{lstlisting}
  \par Die Funktion \texttt{process\_message()} extrahiert zunächst die Geräte-ID aus dem Topic. Anschließend wird der Payload der Nachricht dekodiert und in ein JSON-Objekt data umgewandelt. Diese Daten werden dann über WebSockets an den Benutzer weitergeleitet. Dazu wird die Klasse WebsocketHandler aus der Datei \texttt{services.py} importiert, und die Daten werden an die Funktion \texttt{send\_data()} dieser Klasse übergeben.
  \paragraph{Handshake}
  \par \textbf{}
  \par Die Handshake-Nachricht wird an ein Gerät gesendet, um dessen Konfigurationsdaten zu empfangen. Die Klasse MQTT-Client enthält die Funktion \texttt{send\_handshake()}, die eine MQTT-Nachricht in das Topic „\texttt{command/device\_id}“ mit dem Text „handshake“ sendet. Das Gerät antwortet im Topic „handshake“.
  \par Beim Empfang einer MQTT-Nachricht von diesem Topic verarbeitet die Funktion \texttt{process\_message()} den Inhalt der Nachricht entsprechend. Die empfangenen Daten müssen in die Datenbank eingefügt werden. Dazu werden die Funktionen aus dem Paket „\texttt{db}“ importiert und das gespeicherte Gerät mit den neuen Daten aktualisiert.
  \par Diese Vorgehensweise kann jederzeit durchgeführt werden. Damit ist sichergestellt, dass die Daten bei Bedarf aktualisiert werden können.
  % „“
  \paragraph{Szenario}
  \par \textbf{TODO}
  ...
  \par \textbf{}
  \par Die Datei „\texttt{index.html}“, die sich im Verzeichnis templates befindet (Listing \ref{lst:indexhtml}), bildet die Grundlage für die Benutzeroberfläche der Anwendung. Sie wird vom Server als Antwort auf einer \texttt{GET}-Anfrage an die Rooot-Route „/“ bereitgestellt. 
  \par Im \texttt{head}-Bereich der Datei sind der Seitentitel sowie die Verknüpfung zur CSS-Datei „\texttt{output.css}“ definiert, die das Styling der Anwendung steuert. 
  ...
  \begin{lstlisting}[style=htmlcssjs, label={lst:indexhtml}, caption={index.html}]
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
