...
% 
  ...
  \par Die Funktionalität des Frontends wird durch ein JavaScript-Skript „\texttt{script.js}“ gesteuert. Es ist das dynamische Laden des Inhalts sowie für Benutzerinteraktion verantwortlich. 
  \paragraph{Asynchronen Serveranfragen}
  \par \textbf{NAME DES ABNSCHNITTS ÄNDERN}
  ...
  \begin{lstlisting}[frame=single, style=htmlcssjs, label={lst:jsgetuserdata}, caption={get\_user\_data}]
...
<body>
  ...
  \par \textbf{}
  \par Beim Laden der Seite überprüft die Anwendung, ob ein gültiges Token im \texttt{localStorage} vorhanden ist. Falls kein Token existiert oder es ungültig ist, wird der Nutzer automatisch zur Login-Seite weitergeleitet. Andernfalls wird eine Anfrage an die Route „\texttt{/get\_user\_data}“ gesendet, um die Benutzerdaten abzurufen.
  \par Die Abbildung \ref{abb:login} zeigt die Login-Seite. Die Generierung der Seite erfolgt mit der Funktion „\texttt{renderLoginForm()}“. Dabei wird das \texttt{div}-Element „\texttt{app}“ mit einem Formular versehen, das Eingabefelder für den Benutzernamen und das Passwort enthält. Durch Drücken des Login-Buttons werden die Anmeldedaten an die Route „\texttt{/login}“ gesendet. Stimmen die übermittelten Anmeldedaten mit den hinterlegten Benutzerinformationen überein, antwortet der Server mit einem JWT-Token. Dieses Token wird im localStorage gespeichert, wodurch der Benutzer authentifiziert und angemeldet wird. Andernfalls wird eine Meldung ausgegeben, dass die Anmeldedaten ungültig sind.
  \begin{figure}[h]
    \centering
    ...
    \label{abb:login}
  \end{figure}
  \par Unter dem Login-Button befindet sich ein Link zur Registrierungsseite. Ein Klick auf diesen Link ruft die Funktion \texttt{renderSignUpPage} auf, die das \texttt{div}-Element „\texttt{app}“ mit einem Formular für die Benutzerregistrierung füllt. Die Abbildung \ref{abb:signuppage} zeigt die Registrierungsseite. Das Formular enthält Eingabefelder für den Benutzernamen, die E-Mail-Adresse und das Passwort. Durch Drücken der Taste „Sign Up“ werden die Anmeldedaten an die Route „\texttt{/sign\_up}“ gesendet. Wenn die Registrierungsdaten gültig sind, antwortet die Route mit einem JWT-Token und der Benutzer ist im System angemeldet.
  \begin{figure}[h]
    \centering
    \includegraphics[width=0.75\textwidth]{signup.png}
    \caption{Signup-Seite}
    \label{abb:signuppage}
  \end{figure}
  ...
  \begin{lstlisting}[style=htmlcssjs, label={lst:mainhtml}, caption={script.js: renderMainPage}]
    <div id="main_page">
        <div id="main_header">
  ...
  \begin{lstlisting}[style=htmlcssjs, label={lst:houseelement}, caption={script.js: house\_element}]
  ...
  \end{lstlisting}
  \par Im \texttt{header}-Element wird der Name des Hauses angezeigt. Unterhalb des Headers befindet sich ein Button zur Erstellung eines neuen Raumes sowie der Bereich für die Raumauflistung. Zusätzlich gibt es eine Schaltfläche zum Löschen des Hauses. Die Schaltfläche „\texttt{create\_room}“ öffnet ein Formular, in dem der Benutzer den Namen des Raumes eingeben und speichern kann.
  \par Anschließend wird das \texttt{div}-Element „room\_list“ mit den Räumen des Hauses gefüllt. Dieser Vorgang ist ähnlich wie bei der Anzeige der Häuser. Für jeden Raum wird ein \texttt{div}-Element „room\_element“ mit der ID des Raumes erstellt. Dieses Element enthält den Namen des Raumes, eine Schaltfläche zum Hinzufügen eines Geräts, den Bereich zur Auflistung der Geräte und eine Schaltfläche zum Löschen des Raumes. Die Schaltfläche zum Hinzufügen eines Geräts öffnet ein Formular, in das der Benutzer die ID und den Namen des Geräts eingeben soll. Durch bestätigen der „Save“-Taste wird das Gerät gespeichert und zu dem Raum zugewiesen. 
  \par 
  % „“
\newpage
