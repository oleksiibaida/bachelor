...
\lstdefinelanguage{JavaScript}{
  morekeywords={typeof, const, of, let, new, true, false, catch, function, return, null, catch, switch, var, if, in, while, do, else, case, break},
  morecomment=[s]{/*}{*/},
  morecomment=[l]//,
  morestring=[b]",
  morestring=[b]',
...
% „“
  ...
  \paragraph{FastAPI}
  \par \textbf{}
  \par \texttt{FastAPI} ist ein modernes Webframework für Python, das speziell für die Entwicklung leistungsstarker, asynchroner Webanwendungen und APIs konzipiert wurde. Es basiert auf Starlette für das Web-Handling und Pydantic für die Datenvalidierung
  ...
  \par \textbf{TOODOO} kurzer Überblick auf die Webentwicklung mit HTML/CSS und JavaScript
...
% 
  ...
  \begin{itemize}
    \item \texttt{/} (GET): Root-Endpunkt. Antwortet mit der HTML-Datei „index.html“.
    \item \texttt{/login} (POST): Prüft, ob die übergebene Benutzername-Passwort-Kombination mit der in der Datenbank gespeicherten übereinstimmt. Gibt im Erfolgsfall ein Token zurück. \textbf{Parameter:} 
    \begin{itemize}
        \item \texttt{user\_data}: \{\texttt{username} (str), \texttt{password} (str)\}
    \end{itemize}  
    \item \texttt{/sign\_up} (POST): Fügt einen neuen Benutzer mit den übergebenen Benutzerdaten in die Datenbank ein. \textbf{Parameter:} 
    \begin{itemize}
        \item \texttt{user\_data}: \{\texttt{username} (str), \texttt{password} (str), \texttt{email} (str)\}
    \end{itemize}  
    \item \texttt{/get\_user\_data} (GET): Liefert die in der Datenbank gespeicherten Benutzerdaten als JSON-Objekt zurück. \textbf{Parameter:} 
    \begin{itemize}
        \item \texttt{token} (str)
    \end{itemize}
    \item \texttt{/add\_house} (POST): Fügt ein neues Haus in die Datenbank ein. \textbf{Parameter:} 
    \begin{itemize}
        \item \texttt{token} (str), \texttt{house\_name} (str)
    \end{itemize}  
\end{itemize}
  ...
  \begin{itemize}
    ...
    \item \texttt{/login} (POST):  Prüft, ob die übergebene Benutzername-Passwort-Kombination mit der in der Datenbank gespeicherten übereinstimmt. Im Erfolgsfall gibt ein Token zurück. \textbf{Parameter:} 
      \begin{itemize}
        \item \texttt{user\_data}: \{\texttt{username}: str, \texttt{password} : str\}
      \end{itemize}  
    \item \texttt{/sign\_up} (POST): Fügt der neue Benutzer mit übergebenen Benutzerdaten in die Datenbank ein. \textbf{Parameter:} 
      \begin{itemize}
        \item \texttt{user\_data}: \{\texttt{username}: str, \texttt{password} : str, \texttt{email}: str\}
      \end{itemize}  
    \item \texttt{/get\_user\_data} (GET): Liefert die in der Datenbank gespeicherten Benutzerdaten als JSON-Objekt. \textbf{Parameter:} 
    \begin{itemize}
      \item \texttt{token}: str
    \end{itemize}
    \item \texttt{/add\_house} (POST): Fügt ein neues Haus in die Datenbank ein. \textbf{Parameter:} 
...
\begin{thebibliography}{20}
  ...
  \bibitem{sqlalchemy}
  Mike Bayer, \textit{SQLAlchemy} \url{https://www.sqlalchemy.org}  am: 11. März 2025. [Online].
